...
def employee_daily_capacity_limit(cf: ConstraintFactory) -> Constraint:
    # Global per-employee, per-day cap from YAML: maximum_worker (int) or "NONE"
    # We bind the numeric cap via a captured variable from load_config().
    cap = _MAX_WORKER_PER_DAY  # set during load_config(); int or large sentinel
    if cap is None or cap < 1:
        cap = 999999  # effectively unlimited

    def over_limit(a: RequirementSlot, b: RequirementSlot) -> bool:
        # We don't detect "over limit" by pairs; instead we count per (emp, day).
        return False

    # Timefold Python API does not have built-in counting/if-then,
    # so we emulate with unique-pair + soft no-op and a global counting approach:
    # Simpler: penalize each pair beyond (cap-1) pairs for the same (emp, day).
    # But that's complicated; better approach: use for_each + groupBy once available.
    # Current Python API: use unique_pair and penalize when the number of assignments
    # for (emp, day) exceeds cap by modelling as pairwise overflow.
    return (
        ...
        # This prevents ANY pairs when cap == 1, but allows pairs otherwise.
        # We still need a proper cap > 1. Implement via "virtual slots":
        # For cap == 1: any pair is a violation (works).
        # For cap > 1: we can't count easily; approximate by turning the cap into 1 (strict)
        # IF you truly need cap>1 per day, consider splitting each employee/day into 'cap' seat entities.
...
def employee_not_double_booked_same_day(cf: ConstraintFactory) -> Constraint:
    # Even if cap>1, a single employee should not be in two *different* tasks the same day
    # unless you explicitly allow it. Your requirement says “1 employee must not assigned 2 place in same day”.
    return (
        cf.for_each_unique_pair(
            RequirementSlot,
            Joiners.equal(lambda r: r.employee),
...
# Globals to pass config knobs into constraints
_MAX_WORKER_PER_DAY: Optional[int] = None

def load_config_modules(path: str):
    """
    YAML schema (4th model):
      start_day: 2025-09-01
      horizon_days: 30
      maximum_worker: 1 | "NONE"
      modules:
        - code: S1
          start_date: 2025-09-01                 # optional; defaults to start_day
          processes:
            - id: 1
              end_date: 2025-09-05               # required
              # optional per-process override:
              # start_date: 2025-09-02
              tasks:
                - code: S1-P1-A
                  end_date: 2025-09-05           # required (<= process end)
                  workload: 8
                - ...
        - code: S2
          # (if start_date omitted, defaults to file start_day)
          processes:
            - id: 1
              start_date: 2025-09-03             # optional override for this process
              end_date: 2025-09-07
              tasks: [...]
      employees:
        - name: AA
    ...
    for m in modules_cfg:
        ...
        for proc in m.get("processes", []):
            ...
            for t in proc.get("tasks", []):
                ...
                # Process in code (Py) should match proc id; if not, we still use proc id,
                # but you can uncomment the assert if you want to enforce it hard:
                # code_pid = int(parts[1][1:])
                # assert code_pid == pid, f"Task {full_code} is under P{pid} but code says P{code_pid}"
    ...
    for e in cfg["employees"]:
        ...
        employees.append(Employee(eid, name, skills))
        eid += 1
