...
## 6. Process lifecycle — closing the app ends the session (reverted from the original plan)

**Status: reverted, 2026-08-28.** This section originally specified hiding the window to a tray icon (keeping the embedded server alive) instead of quitting when a session was active, so a host closing their window wouldn't end the session. That was built (Task 12) and shipped, but field feedback surfaced two real problems: the single-instance relaunch handler only checks `isMinimized()` and never un-hides a window that was `.hide()`-d, so reopening the app after closing it mid-session silently did nothing (looked broken); and a user who closes the window without realizing it's still running invisibly leaves the app — and its embedded server — consuming resources in the background indefinitely, with no obvious way to notice or stop it.

Given both problems trace back to the same root cause (a window that's alive but invisible, with no clear affordance back), the fix for now is to remove the hide-to-tray behavior entirely rather than patch around it: closing the window, or quitting the app, always stops the embedded server — exactly the behavior from before this section existed. **Host leaving now does end the session** — the earlier "no outsourced/cloud server for v1, keeps running on whichever machine started it, self-hosted" framing still holds, it just means that machine's *app* has to stay open, not merely the machine itself.

A real fix for "survive the host closing their window" — done properly, with a visible, discoverable way back rather than a silent background process — is a candidate for later, once there's a clear design for making the running-in-background state obvious to the user (a persistent, unmissable indicator; a relaunch path that actually restores the hidden window; explicit user opt-in rather than an implicit side effect of having a session open). Until then, the standalone-server path already described below is the recommended way to get resilience against any one person's app closing: run the server as its own process (`node dist/index.js`) on a machine dedicated to staying on, separate from anyone's editor — no client-side or protocol change needed for that, the server was already designed to be a standalone connection target.
