...
def define_constraints(cf: ConstraintFactory) -> List[Constraint]:
    return [
        ...
        # NEW HARD: enforce min/max people per (module,process,task,day)
        staffing_below_min(cf),
        staffing_above_max(cf),
...
def staffing_below_min(cf: ConstraintFactory) -> Constraint:
    """
    For each (module, process, task, day), require at least _STAFF_MIN_PER_DAY people.
    If fewer than the minimum, penalize the shortfall as HARD.
    """
    return (
        cf.for_each(RequirementSlot)
        .filter(lambda r: r.day is not None)
        .group_by(
            # group key: (module, process_id, task_letter, day_id)
            lambda r: (r.module, r.process_id, r.task_letter, r.day.id),
            ConstraintCollectors.count()
        )
        .filter(lambda key, count: count < _STAFF_MIN_PER_DAY)
        .penalize(HardSoftScore.ONE_HARD, lambda key, count: _STAFF_MIN_PER_DAY - count)
        .as_constraint("Daily staffing below minimum")
    )


def staffing_above_max(cf: ConstraintFactory) -> Constraint:
    """
    For each (module, process, task, day), cap at _STAFF_MAX_PER_DAY people.
    If more than the maximum, penalize the overflow as HARD.
    """
    return (
        cf.for_each(RequirementSlot)
        .filter(lambda r: r.day is not None)
        ...
        .filter(lambda key, count: count > _STAFF_MAX_PER_DAY)
        .penalize(HardSoftScore.ONE_HARD, lambda key, count: count - _STAFF_MAX_PER_DAY)
        .as_constraint("Daily staffing above maximum")
    )
