...
def define_constraints(cf: ConstraintFactory) -> List[Constraint]:
    return [
        ...
        # HARD: staffing min/max (combined)
        # staffing_minmax_combined(cf),
        staffing_min_at_least_two(cf),                 # ≈ enforce min=2
        staffing_max_pairwise_quadratic(cf, 8),        # ≈ cap near 8
        # HARD: country travel gap
        country_changeover_gap(cf),

        # HARD: visa (consecutive stay) per country
        visa_presence_limit(cf),

        # HARD: annual presence (segment span) per country
...
def staffing_min_at_least_two(cf: ConstraintFactory) -> Constraint:
    """
    HARD (approx., no collectors):
    For each assigned RequirementSlot r, require that there exists a *different*
    employee on the same (module, process, task, day). If not, penalize.
    - When N=0: nothing to check (other hard constraints will push to assign).
    - When N=1: exactly one r -> penalty 1.
    - When N>=2: no penalty (condition satisfied).
    """
    return (
        cf.for_each(RequirementSlot)
        .filter(lambda r: r.day is not None and r.employee is not None)
        .if_not_exists(
            RequirementSlot,
            # same module / process / task / day
            Joiners.equal(lambda a: a.module,     lambda b: b.module),
            Joiners.equal(lambda a: a.process_id, lambda b: b.process_id),
            Joiners.equal(lambda a: a.task_letter,lambda b: b.task_letter),
            Joiners.equal(lambda a: a.day.id if a.day else None,
                          lambda b: b.day.id if b.day else None),
            # but a *different* employee
            Joiners.filtering(lambda a, b: b.employee is not None and
                                         a.employee.id != b.employee.id)
        )
        .penalize(HardSoftScore.ONE_HARD)
        .as_constraint("Daily staffing min≈2 (no collectors)")
    )


def staffing_max_pairwise_quadratic(cf: ConstraintFactory, approx_max: int = 8) -> Constraint:
    """
    HARD (approx., no collectors):
    Penalize each *pair* of employees on the same (module, process, task, day).
    Because #pairs = C(N, 2), the penalty grows quadratically and strongly
    discourages large piles. To roughly align with a cap K, we scale the weight
    so that up to K people is 'tolerable' and above K becomes painful.

    NOTE: This is an approximation — it doesn't create a precise 'no more than K'
    feasibility wall, but it works well in practice and avoids collectors.
    """
    # Choose a weight so C(K,2) is the "neutral" zone. You can tune this number.
    # If you want it harsher above K, increase weight; if too strict, reduce it.
    pair_weight = 1
    ...
    return (
        ...
        .penalize(HardSoftScore.of_hard(pair_weight))
        .as_constraint(f"Daily staffing max≈{approx_max} via pairwise penalty (no collectors)")
    )
...
def _segments_from_days(day_ids: List[int], break_days: int) -> List[Tuple[int, int]]:
    ...
    assignments less than 'break_days' apart belong to the same 'stay'.
    Each segment is (first_day_id, last_day_id) and span is inclusive (calendar days).
    """
    if not day_ids:
        return []
    ...
    prev = s[0]
    for d in s[1:]:
        # if gap >= break_days, start new segment
        if (d - prev) >= break_days:
...
def visa_presence_limit(cf: ConstraintFactory) -> Constraint:
    """
    For each (employee, country) stay segment (split by >= _VISA_GAP_BREAK_DAYS),
    penalize if calendar span exceeds _VISA_LIMITS[country].
    """
    def key_emp_country(r: RequirementSlot):
        if r.employee is None or r.day is None:
            return None
        ctry = _MODULE_COUNTRY.get(r.module, None)
        if ctry is None or ctry == "":
            return None
        return (r.employee.id, ctry)

    def overstay_penalty(days: List[int], ctry: str) -> int:
        limit = _VISA_LIMITS.get(ctry, None)
        if not limit or not days:
            return 0
        penalty = 0
        for a,b in _segments_from_days(days, max(1, _VISA_GAP_BREAK_DAYS)):
            span = (b - a) + 1  # calendar days inclusive
            if span > limit:
                penalty += (span - limit)
        return penalty
    ...
    return (
        ...
        .group_by(key_emp_country,
                  ConstraintCollectors.to_list(lambda r: r.day.id))
        .filter(lambda key, day_list: key is not None)
        .penalize(HardSoftScore.ONE_HARD,
                  lambda key, day_list: overstay_penalty(day_list, key[1]))
        .as_constraint("Visa stay limit (per country)")
    )

def annual_presence_limit(cf: ConstraintFactory) -> Constraint:
    """
    For each (employee, country), build segments split by >= _ANNUAL_BREAK_DAYS (calendar).
    Penalize span overflow against _ANNUAL_LIMITS[country].
    """
    def key_emp_country(r: RequirementSlot):
        if r.employee is None or r.day is None:
            return None
    ...
    def over_annual_penalty(days: List[int], ctry: str) -> int:
        limit = _ANNUAL_LIMITS.get(ctry, None)
        if not limit or not days:
            return 0
        penalty = 0
        for a,b in _segments_from_days(days, max(1, _ANNUAL_BREAK_DAYS)):
            span = (b - a) + 1
            if span > limit:
                penalty += (span - limit)
        return penalty
    ...
    return (
        ...
        .filter(lambda key, day_list: key is not None)
        .penalize(HardSoftScore.ONE_HARD,
                  lambda key, day_list: over_annual_penalty(day_list, key[1]))
...
def solve_from_config(cfg_path: str = "config_modules.yaml"):
    ...
    audit_staff_caps(solution, start_day)
