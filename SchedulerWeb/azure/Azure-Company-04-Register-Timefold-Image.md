...
| `denied: requested access to the resource is denied` on push, even though you have `Contributor` on the ACR | `Contributor` doesn't grant push/pull — those are data-plane actions, deliberately excluded (see [Azure-Company-02-RBAC.md](./Azure-Company-02-RBAC.md#contributor-is-not-enough--read-this-before-you-check-anything)) | Ask the admin for `AcrPush` specifically — `Contributor` can never cover this, no matter the scope |
| `denied: requested access to the resource is denied` on push, and `AcrPush` is confirmed assigned | Your `az acr login` token expired (valid ~24h)          | Re-run `az acr login --name "$ACR"` |
