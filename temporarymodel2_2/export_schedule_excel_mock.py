...
def write_sheet_tasks_dates(wb, plan_start, plan_end, env, maps, modules, vios, req_task,
    ...
    fill_weekend = PatternFill(start_color=GREY_WEEKEND, end_color=GREY_WEEKEND, fill_type="solid")
    ...
    for r_idx, (m_id, op_id, op_name) in enumerate(rows, start=2):
        ...
        for j, dt in enumerate(dates, start=len(headers)+1):
            items = maps["tde"].get((m_id, op_id, dt), [])
            txt = " | ".join(sorted(items)) if not (skip_weekends and dt.weekday() >= 5) else ""
            ...
            # decor (non-violation markers)
            if m_start and dt == m_start:
                c.fill = fill_start if dt.weekday() < 5 else fill_weekend
            if p_end and dt == p_end:
                c.fill = fill_deadln if dt.weekday() < 5 else fill_weekend

            # violations overlays (only if not weekend)
            key = (m_id, op_id, dt)
            if dt.weekday() < 5:
...
def write_sheet_employees_dates(wb, plan_start, plan_end, env, maps, vios,
    ...
    for i, (comp, wname) in enumerate(roster, start=2):
        ...
        for j, dt in enumerate(dates, start=len(base_headers)+1):
            items = maps["edt"].get((comp, wname, dt), [])
            txt = " | ".join(sorted(items)) if not (skip_weekends and dt.weekday() >= 5) else ""
            c = ws.cell(row=i, column=j, value=txt); c.alignment = center

            if dt.weekday() >= 5:
                c.fill = fill_weekend
            elif (comp, wname, dt) in vios["skill_mismatch_cells"]:
                c.fill = fill_skill
...
def main():
    ...
    ap.add_argument("--skip-weekends", action="store_true", default=True,
                help="Grey out Sat/Sun and suppress showing any weekend work in grids.")
    args = ap.parse_args()
