...
def _build_pass1_and_solve(day_slots: List[DaySlot], windows: List[TaskWindow],
    ...
    # ----------------- EVIDENCE SWITCH -----------------
    # True  -> try LS/Tabu + custom moveSelector (will raise on 1.24.0b; use for screenshots)
    # False -> use minimal supported config (no LS/Tabu), so you can actually run a solve.
    EVIDENCE_MODE = True
    # ---------------------------------------------------

    if EVIDENCE_MODE:
        # Intentionally use kwargs that 1.24.0b does NOT support so the run fails with a real error.
        print("[EVIDENCE] Attempting Local Search + Tabu + move selectors (expected to FAIL on 1.24.0b).")
        cfg = SolverConfig(
            ...
            # This key ('phase_list') + the LS section below is what should trigger the TypeError.
            phase_list=[
                {
                    "phaseType": "CONSTRUCTION_HEURISTIC",
                    "constructionHeuristicType": "FIRST_FIT"
                },
                {
                    "phaseType": "LOCAL_SEARCH",
                    "moveSelectorConfig": {
                        "unionMoveSelectorConfig": {
                            "moveSelectorConfigList": [
                                { "changeMoveSelectorConfig": {
                                    "entityClass": BlockDecision, "variableName": "start_day"
                                }},
                                { "changeMoveSelectorConfig": {
                                    "entityClass": BlockDecision, "variableName": "heads"
                                }},
                                { "changeMoveSelectorConfig": {
                                    "entityClass": BlockDecision, "variableName": "days"
                                }},
                                { "swapMoveSelectorConfig": { "entityClass": BlockDecision } }
                            ]
                        }
                    },
                    "acceptorConfig": { "entityTabuSize": 7, "valueTabuSize": 7 },
                    "foragerConfig": { "acceptedCountLimit": 4 }
                }
            ],
        ...
        # On 1.24.0b, constructing the config above should already raise.
        # If not, the following line will, and you'll still get a real traceback:
        solver = SolverFactory.create(cfg).build_solver()
        solved: Pass1Plan = solver.solve(pass1)  # very unlikely to be reached on 1.24.0b
        print(f"[EVIDENCE] (unexpected) LS/Tabu ran; score={solved.score}")
        return solved.blocks

    # -------- Minimal supported config (works on 1.24.0b) --------
    print("[RUN] Using minimal supported SolverConfig (no LS/Tabu/move selectors).")
    cfg = SolverConfig(
        solution_class=Pass1Plan,
        entity_class_list=[BlockDecision],
        score_director_factory_config=ScoreDirectorFactoryConfig(
