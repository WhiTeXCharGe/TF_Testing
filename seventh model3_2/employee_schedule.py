...
def _build_solver(solution_cls, entity_cls_list, constraint_fn,
    ...
    # ⬇️ cast to int — Duration only accepts long/int
    if spent_minutes and spent_minutes > 0:
        term_kwargs["spent_limit"] = Duration(minutes=int(spent_minutes))
...
def solve_from_yaml(env_path: str = "EnvConfig.yaml",
    ...
    # soft balancing target for Pass 2 (used by p2_soft_balance_total_hours)
    ...
    if not p1_ok:
        print("PASS 1 hard!=0 after ramp — STOP. (No Pass 2 run.)")
        # Build a minimal object for optional debugging/export if you want:
        seats, seat_days = _expand_blocks_to_seats_and_days(blocks, day_slots)
        class FinalMinimal: pass
        final = FinalMinimal(); final.seats = seats; final.seat_days = seat_days
        # comment-out export by default when hard!=0
        # overwrite_schedule_with_assignments(final, start_day, "Schedule.yaml")
        return final, start_day

    # ---- Expand to seats for Pass 2 ----
    seats, seat_days = _expand_blocks_to_seats_and_days(blocks, day_slots)

    # ---- PASS 2 (single run, then optional polish) ----
    final_p2 = _solve_pass2_once_with_optional_polish(
        day_slots=day_slots,
        employees=employees,
        seats=seats,
        seat_days=seat_days,
    ...
    # ---- Export result (Pass 2 plan has seats & seat_days) ----
    overwrite_schedule_with_assignments(final_p2, start_day, "Schedule.yaml")
    return final_p2, start_day
