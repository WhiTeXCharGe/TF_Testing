...
public class EmployeeSchedule {
    ...
    public static class CrewSeat {
        ...
        @PlanningPin
        public boolean isPinned() { return pinnedFixed || warmPinned; }
    ...
    // - FIXED + warmPinned flexible seats (stage1 only)
    static Map<Integer, List<WorkSeg>> BASE_STAGE1_SEGS_BY_EMP = new HashMap<>();

    // Pointer: whichever baseline you want constraints to use "right now"
    static Map<Integer, List<WorkSeg>> BASE_SEGS_BY_EMP = new HashMap<>();
    ...
    public static class SinglePassConstraints implements ConstraintProvider {
        ...
        // Use pinned hours when pinned (fixed or warmPinned), else block hours
        ...
        private static List<WorkSeg> allSegsForEmp(int empId, List<WorkSeg> dyn) {
            ...
            List<WorkSeg> base = BASE_SEGS_BY_EMP.get(empId);
    ...
    static Map<Integer, List<WorkSeg>> buildWarmPinnedWorkSegsFromFlexibleRows(ParsedSchedule sch, ParsedEnv env) {
        Map<Integer, List<WorkSeg>> out = new HashMap<>();
        if (sch.flexibleRows == null) return out;

        for (FlexibleAssign fa : sch.flexibleRows) {
            if (fa == null || fa.wid == null || fa.wid.isBlank()) continue;
            if (fa.hoursByDay == null || fa.hoursByDay.isEmpty()) continue;
    ...
    static Map<Integer, List<WorkSeg>> mergeSegMaps(Map<Integer, List<WorkSeg>> a,
                                                Map<Integer, List<WorkSeg>> b) {
        Map<Integer, List<WorkSeg>> out = new HashMap<>();
        if (a != null) {
    ...
    public static RunResult solveFromYaml(String envPath, String schedPath) throws IOException {
        ...
        // Stage1 baseline includes warmPinned flexible seats
        Map<Integer, BlockDecision> byBlock = new HashMap<>();
        for (BlockDecision b : built.blocks) byBlock.put(b.id, b);

        Map<Integer, List<WorkSeg>> warmPinned = buildWarmPinnedWorkSegsFromFlexibleRows(sch, env);
        BASE_STAGE1_SEGS_BY_EMP = mergeSegMaps(BASE_FIXED_SEGS_BY_EMP, warmPinned);
        // Use stage1 baseline now
        BASE_SEGS_BY_EMP = BASE_STAGE1_SEGS_BY_EMP;
        // --------------------------------------------------
        ...
        // Stage2 should treat baseline as fixed-only
        BASE_SEGS_BY_EMP = BASE_FIXED_SEGS_BY_EMP;
