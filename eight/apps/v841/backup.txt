...
public class EmployeeSchedule {
    ...
    static final int MODULE_HISTORY_LOOKBACK_DAYS = 5;
    ...
    static ParsedSchedule parseSchedule(String schedPath, Map<String,OpDef> opdef) throws IOException {
        ...
        // NEW: modules whose last phase ends BEFORE this date are treated as "history only" and ignored
        LocalDate moduleKeepCutoff = start.minusDays(MODULE_HISTORY_LOOKBACK_DAYS);
        ...
        // Remember which modules are still in scope so we can also filter assignments.
        Set<String> keptModules = new HashSet<>();
        ...
        for (Map<String,Object> wf : wfTasks) {
            ...
            List<Map<String,Object>> phases = (phasesObj instanceof List)
                    ? (List<Map<String,Object>>) phasesObj
                    : List.of();

            // --- NEW: compute last end date of this module ---
            LocalDate moduleLastEnd = null;
            for (Map<String,Object> ph : phases) {
                LocalDate pEnd = LocalDate.parse(
                        safeStr(ph.get("end_date")).replace("-", "/"), DF);
                if (moduleLastEnd == null || pEnd.isAfter(moduleLastEnd)) {
                    moduleLastEnd = pEnd;
                }
            }

            // If the whole module finished long before the current plan_start, drop it completely
            if (moduleLastEnd != null && moduleLastEnd.isBefore(moduleKeepCutoff)) {
                continue;
            }

            keptModules.add(module);

            // --- Build TaskWindow for each phase / operation ---
            for (Map<String,Object> ph : phases) {
                ...
                // NEW: clamp windows to the current horizon [0 ..]
                if (endId < 0) {
                    // Phase is fully before the new plan_start -> treat residual work as starting now
                    startId = 0;
                    endId = 0;
                } else if (startId < 0) {
                    // Phase overlaps the boundary -> cut off the past part
                    startId = 0;
                }

                Object opsObj = ph.get("operation_task_list");
                List<Map<String,Object>> opTasks = (opsObj instanceof List)
                        ? (List<Map<String,Object>>) opsObj
                        : List.of();
        ...
        // ---- Read fixed assignments (only for kept modules) ----
        ...
        for (Map<String,Object> a : asgs) {
            ...
            // NEW: skip assignments for modules completely out of scope
            if (!keptModules.contains(module)) {
                continue;
            }
            ...
            List<Map<String,Object>> wdl = (a.get(wdKey) instanceof List)
