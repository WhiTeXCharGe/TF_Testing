...
def create_new_modules(
    ...
    Here we just:
      ...
      - space them roughly one working day apart.
    """
    if num_to_add <= 0:
        return [], start_day
    ...
    for offset in range(num_to_add):
        ...
        # next candidate start: next calendar day, skipping holidays
        next_day = eq_start + dt.timedelta(days=1)
...
def main():
    ...
    total_target = int(cfg.EQ_NUM)
    remaining = max(0, total_target - current_n)
    ...
    # How many new modules THIS RUN will try to add?
    extra = 0
    if remaining > 0:
        mean = float(getattr(base, "EQ_PER_DAYS", 1.0))
        sigma = float(getattr(base, "EQ_PER_DAYS_SIGMA", 0.0))
        # draw a non-negative integer; may be 0 -> no new modules today
        draw = max(0.0, random.gauss(mean, sigma))
        extra = min(remaining, int(round(draw)))

    if extra > 0:
        # prefer today's date if provided, otherwise cutoff date
        today_str = getattr(cfg, "CURRENT_SIM_DAY_STR", None)
        if today_str:
            start_day = parse_date(today_str)
        else:
            start_day = cutoff

        # do not go backwards before the latest module start
        if last_start is not None and start_day < last_start:
            start_day = last_start
