...
public class EmployeeSchedule {
    ...
    static ParsedSchedule parseSchedule(String schedPath, Map<String,OpDef> opdef) throws IOException {
        ...
        if (TRIM_FINISHED_MODULES) {
            ...
            for (TaskWindow w : windows) {
                // w.startDayId / w.endDayId are still in the original 0-based plan_range coordinates
                w.startDayId = Math.max(w.startDayId, cutOffDayId);
                if (w.startDayId > w.endDayId) {
                    // no room left in this horizon
                    System.out.printf(
    ...
    static <S> SolverFactory<S> buildSolverFactory(
        ...
        // ----- Phase 1a: CH for BlockDecision -----
        ConstructionHeuristicPhaseConfig chBlocks =
                buildChForEntity(BlockDecision.class);

        // ----- Phase 1b: CH for CrewSeat -----
        ConstructionHeuristicPhaseConfig chSeats =
                buildChForEntity(CrewSeat.class);
