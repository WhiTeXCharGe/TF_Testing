...
def build_maps(env, modules, assignments):
    ...
    for a in assignments:
        # NEW (this rev): never render weekend work on Sheets 1/2/3 → skip mapping if weekend
        if _is_weekend(a["date"]):               # <-- suppress any text on weekends in display sheets
            continue
...
def detect_violations(env, modules, assignments, maps, cal, plan_start=None, plan_end=None):
    ...
    # ===== NEW: Region presence & move-gap checks =====
...
def write_sheet_employees_dates(wb, plan_start, plan_end, env, maps, vios, cal):
    ...
    for i, (comp, wname) in enumerate(roster, start=2):
        ...
        for j, dt in enumerate(dates, start=len(base_headers)+1):
            ...
            # ----- GREY OUT weekend + personal & worker-company off -----
            is_weekend   = dt in cal["weekends"]
            personal_off = bool(wid) and (dt in cal["worker_off"].get(wid, set()))
            company_off  = bool(wco) and (dt in cal["worker_company_off"].get(wco, set()))
            if is_weekend or personal_off or company_off:
                c.fill = fill_grey
            # ------------------------------------------------------------
