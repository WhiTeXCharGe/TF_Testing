...
def detect_violations(env, modules, assignments, maps, cal, plan_start=None, plan_end=None):
    ...
    for a in assignments:
        ...
        # Task-side closures (fab/region/customer only; no global weekend anymore)
        reasons = []
        ...
        # ----------  preference breaches (prefer level == 0) ----------
        ...
        if reason_tags:
            ...
            # avoid duplicating many rows for same worker-module-reason
    ...
    for (comp, wname, dt), lst in maps["per_cell_assigns_emp"].items():
        ...
        for info in lst:
            ...
            # NEW: accept both 'p1o1' and 'm1p1o1' style keys in skill_map
    ...
    # ===== NEW: Overtime limit breaches (monthly / annual) =====
    workers       = env["workers"]
    wcompanies    = env["worker_companies"]
    worker_name   = maps["worker_name"]
    ...
    # Aggregate OT (hours above OT_THRESHOLD) by year/month
