...
def _segments_from_days(day_ids: List[int], break_days: int) -> List[Tuple[int, int]]:
    """
    Build segments of continuous presence in a country.
    A new segment starts only if there are >= break_days consecutive calendar days
    NOT in that country between two same-country days.
    """
    ...
    cur_start = s[0]
    prev = s[0]
    bd = max(0, int(break_days))  # allow 0
    for d in s[1:]:
        # non-country days between prev and d:
        non_ctry_gap = (d - prev - 1)
        if non_ctry_gap >= bd:
...
def country_changeover_gap(cf: ConstraintFactory) -> Constraint:
    """
    HARD: When an employee switches from country C1 to C2, the next assignment in C2
    must be at least `gap` days after the day in C1 (i.e. there must be gap 'empty' days
    between the two working days).
    Violation if (b.day.id - a.day.id) <= gap for the ordered pair (a before b).
    """
    def violates(a: RequirementSlot, b: RequirementSlot) -> bool:
        # Must be fully assigned pairs for the same employee
        if a.employee is None or b.employee is None or a.day is None or b.day is None:
            return False

        # Order by day (so a is earlier than b)
        if a.day.id > b.day.id:
            a, b = b, a

        # Same country? No changeover needed.
        if a.country == b.country:
            return False

        # Look up required gap for the *direction* a.country -> b.country
        gap = _COUNTRY_CHANGEOVER.get(a.country, {}).get(b.country, 0)
        if gap <= 0:
            return False

        # Require at least 'gap' empty days between working days:
        # violation if b is too soon after a
        return (b.day.id - a.day.id) <= gap

    return (
        cf.for_each_unique_pair(
            RequirementSlot,
            Joiners.equal(lambda r: r.employee)   # same employee
            # NOTE: no less_than joiner; we handle ordering inside
        )
        .filter(violates)
        .penalize(HardSoftScore.ONE_HARD)
...
def solve_from_config(cfg_path: str = "config_mock.yaml"):
    ...
    t0 = time.time()
    solver1 = _build_solver(best_limit="0hard/*soft", spent_minutes=5)  # small guard cap
